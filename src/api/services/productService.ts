...
> {
  ...
  async getAllDetails(): Promise<ApiResponse<Product[]>> {
    const response = await baseClient.get(`/detail/getAllDetails`);
    return response.data;
  }

  async allSubDetails(id: any): Promise<ApiResponse<Product[]>> {
    const response = await baseClient.get(`/subDetails/allSubDetails/${id}`);
    return response.data;
  }

  async filterProduct(postData: any): Promise<ApiResponse<Product[]>> {
    const response = await baseClient.post(`/product/filterProduct`, postData);
    return response.data;
  }
